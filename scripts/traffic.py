...
p=argparse.ArgumentParser();p.add_argument('--count',type=int,default=1000);p.add_argument('--phase',choices=['baseline','incident','fixed'],required=True);p.add_argument('--out',type=Path,default=Path('evidence/requests.jsonl'));args=p.parse_args()
...
with args.out.open('a') as f:
 for i,store in enumerate(stores):
    ...
    req=urllib.request.Request('http://127.0.0.1:8084/checkout',json.dumps(body).encode(),{'Content-Type':'application/json','x-correlation-id':cid})
    start=time.monotonic()
    try:
        with urllib.request.urlopen(req,timeout=20) as r: status=r.status;response=json.load(r)
